...
interface CourtListProps {
  ...
  onStatsUpdate?: (stats: { restaurantCount: number, menuCount: number }) => void
}
...
export default function CourtList({ userLocation, locationLoading, onStatsUpdate }: CourtListProps) {
  ...
  // Update parent with stats whenever they change
  useEffect(() => {
    if (onStatsUpdate) {
      onStatsUpdate({
        restaurantCount: menuStats.total,
        menuCount: menuStats.withCurrentMealMenu // This is the "live" count (today + current meal)
      })
    }
  }, [menuStats, onStatsUpdate])
